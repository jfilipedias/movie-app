...
func (r *Registry) Register(ctx context.Context, serviceName, instanceID, hostPort string) error {
	r.Lock()
	defer r.Unlock()
...
func (r *Registry) Deregister(ctx context.Context, serviceName, instanceID string) error {
	r.Lock()
	defer r.Unlock()
	if _, ok := r.serviceAddrs[serviceName]; !ok {
		return nil
